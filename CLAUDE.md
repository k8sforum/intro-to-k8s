...
- **Image description feature** (`v1.0.7+`): Added `AnthropicImageDescriptionService` that calls Claude to generate POI descriptions. Requires `ANTHROPIC_API_KEY` env var. Wired into all five stages (stages 3–4 also expect a `ANTHROPIC_API_KEY` secret, see `3-kubernetes/manifests/api/1-secret.yaml`).
