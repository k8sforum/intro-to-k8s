...
- Image tags are pinned per stage and drift: the `web` Compose tag (`v1.0.4`) in stage 2 (`docker-compose.yml`) trails the build helper and k8s manifests (`v1.0.7`). Check the tag you're editing against `2-dockerhub/docker-compose.build.yml`, which is what actually gets built and pushed. Note: `v1.0.7+` images are built with sentinel placeholders; stages 3–4 substitute runtime values via ConfigMap (`render-config` init container).
- **Web image runtime configuration** (`v1.0.7+` in stages 3–4): The published `mytravels-web` image is built with sentinel placeholders (`__API_BASE_URL__`, `__OTLP_TRACES_ENDPOINT__`) for both Vite env vars. A `render-config` init container copies the nginx docroot to an `emptyDir`, uses `sed` to substitute values from the `web-config` ConfigMap, and mounts that over the original docroot. This lets stages 3–4 reconfigure the frontend without rebuilding the image. Stages 0–2 (Compose) use earlier `v1.0.4` images built with literal URLs, so they do not support this feature.
- **Image description/tagging feature** (`v1.0.7+`, made async in `v1.0.11+`): `AnthropicImageDescriptionService` (`src/common/mytravels.common/Services/`, implements `IImageDescriptionService`) calls Claude to generate a POI's `Description` and `Tags`. It's invoked only from the `AppendImageTags` subscriber in `messaging` (published via the `append-image-tags` exchange above) — the old synchronous REST describe endpoint and the web app's manual "Describe" button are gone; `PoiDialog` now renders `description`/`tags` straight from POI data. `ANTHROPIC_API_KEY` therefore lives only on `messaging` (Compose env, and the `messaging` secret in `3-kubernetes/manifests/` / `4-argocd/manifests/`) — despite this, `api` and `mcp` still DI-register `IImageDescriptionService` because it's an unused constructor dependency of the shared `PointOfInterestService`, and `mytravels.api/appsettings.json` still carries a stale `AnthropicApiKey` placeholder; don't be misled into thinking `api` uses it.
