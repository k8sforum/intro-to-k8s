...
**MyTravels** lets a user upload a geotagged photo (or pick a location manually) to create a "Point of Interest" pin on a map. GPS coordinates are read from the photo's EXIF data on upload; if absent, the upload is rejected and the user searches for and picks a place instead, posting to a different endpoint. A background worker then asynchronously (a) resizes the uploaded image into a thumbnail, (b) resolves a human-readable address for the coordinates via a maps geocoding API, and (c) generates a description and scene tags for the photo via Claude (Anthropic), persisting all three onto the POI. The frontend is a two-route SPA: a Leaflet map view that polls for updates while async resolution completes, and a traceability view that replays the message lifecycle of each upload by correlation ID.
...
| `mytravels.messaging` | Background worker — image resize, address resolution, AI description/tagging, retry sweep | `src/messaging/mytravels.messaging/` |
...
| `mytravels.common` / `mytravels.contract` / `mytravels.domain` / `mytravels.storage` | Shared libraries (DTOs, entities, EF Core context, geo/maps services, object storage, messaging + audit logging, Anthropic integration) | `src/common/` |
...
The two surfaces are **not feature-equivalent**: the REST API exposes CRUD operations (list, fetch, search, update, upload) plus the traceability read surface, none of which have an MCP counterpart. The MCP server is read-only search: `search_pointofinterest` (search saved POIs by formatted address) and `search_place` (place lookup) — an MCP client can only query existing data, not create or modify anything. The earlier `upload_photo` / `upload_photo_with_coordinates` tools were removed, which also retired the validation-drift and error-handling findings that applied to them (former F-16 and F-17).
...
PointOfInterestService.CreatePointOfInterestAsync
        ...
        │
        └──publish──► fanout "append-image-tags" ──► queue "append-image-tags"
                                                            │
...
All six exchanges are RabbitMQ **fanout**, bound with an empty routing key. The three work exchanges are declared independently by both the publisher and the consumer (no shared topology-setup step); the three `-failed` exchanges are declared `durable: false, autoDelete: true` both by `FailedExchangeDeclarer` (an `IHostedService` in `messaging` that runs once at startup) and again by each `MessageSubscriberBase<T>` — see §17 for what this duplication implies at scale. **Nothing consumes the `-failed` exchanges**: they are fanout with no bound queue, so a dead-lettered `FailedMessage` is dropped on publish. The durable record of the failure is the `MessageAuditLogs` `Failed` row, not the message.
...
│                                 #   prompt.md", "find traceability issues.md", …); todo/ holds specs
│                                 #   not yet implemented — "implement search.md" (SOLR-backed POI
│                                 #   search, untracked) and "traceability gaps spec.md"
...
| `mytravels.messaging` | Async processing | Image resize, address resolution, AI description/tagging, retry sweep, failed-exchange topology | `ResizeImage`, `AppendFormattedAddress`, `AppendImageTags`, `AppendFormattedAddressSweeper`, `FailedExchangeDeclarer` (all `IHostedService`/`BackgroundService`) | common, contract, domain, storage |
...
| `mytravels.common` | Cross-cutting services | Geocoding (Google/OSM), RabbitMQ publish/subscribe base classes (incl. retry, dead-lettering, trace propagation and audit logging), cron scheduling base class, AI image description | `GoogleMapsService`, `OpenStreetMapsService`, `ImageMetadataService`, `AnthropicImageDescriptionService`, `MessagePublisher`, `MessageSubscriberBase<T>`, `CronJobBase` | contract |
| `mytravels.contract` | Shared data shapes | Entities, DTOs, interfaces, exceptions, constants, message payloads | `PointOfInterest`, `Tag`, `MessageAuditLog`, `PointOfInterestMessage`, `FailedMessage`, `*Dto`, `ICoreDbContext`, `IObjectStorageService`, `IMapsService`, `ITraceabilityService`, `IMessageAuditLogger` | none (leaf) |
| `mytravels.domain` | Persistence | EF Core DbContext, migrations, stored-proc invocation, POI business logic, traceability queries | `CoreDbContext`, `PointOfInterestService`, `TraceabilityService`, `MessageAuditLogger` | contract |
...
| GET | `/api/pointofinterest/search?term=` | query `term` | `List<PointOfInterestDto>` | 200 | none |
...
| GET | `/api/traceability?page=&pageSize=` | query `page`, `pageSize` (both coerced to 1 / 25 when ≤ 0) | `List<CorrelationSummaryDto>` | 200 | none |
| GET | `/api/traceability/{correlationId:guid}` | route `correlationId` | `List<MessageAuditLogDto>` (ordered by `CreatedAt`) | 200 (empty list for an unknown id, not 404) | none |
...
|---|---|---|---|
| `search_pointofinterest` | `term` (note: **not** `query`) | `List<PointOfInterestDto>` | `McpException` if `term` is blank. Searches saved POIs by formatted address, grouping stored-proc rows into one DTO per POI with its tags. |
...
**Note:** Photo upload via MCP is not implemented; POI creation and upload are REST-only, driven from the `api` service (`POST /api/pointofinterest/image` and `POST /api/pointofinterest/image/coordinates`). The MCP server is read-only. `PointOfInterestMcpTools.ToDto` does not project `Description` or `CorrelationId`, so MCP search results omit both fields even though the REST DTO carries them.
...
### 6.4 Message-queue interfaces (RabbitMQ, six fanout exchanges; work queues are durable and named after their exchange, with no DLX/TTL/max-length arguments)
...
| `append-image-tags` | `PointOfInterestService.CreatePointOfInterestAsync` | `AppendImageTags` (HostedService) | `PointOfInterestMessage { CorrelationId, PointOfInterestId }` |
...
Exposes `DbSet`s (`PointOfInterests`, `PointOfInterestTagAssociations`, `Tags`, `MessageAuditLogs`) plus domain methods: `GetPointsOfInterestAsync`, `GetPointsOfInterestByTagAsync`, `GetPointsOfInterestByKeyAsync`, `GetAllPointsOfInterestAsync`, `SearchPointsOfInterestByFormattedAddressAsync`, `CreatePointOfInterestAsync`, `UpdatePointOfInterestTagsAsync`, `AddImageToPointOfInterestAsync`, `UpdateAddressAsync`, `GetCorrelationSummariesAsync`, `GetEventsByCorrelationIdAsync`, plus generic `AddObject`/`DeleteObject`/`DetachObject`/`Entry`/`ExecuteSqlInterpolatedAsync`.

Note the search asymmetry: `GetAllPointsOfInterestAsync` and the by-tag/by-key reads go through stored procedures, but `SearchPointsOfInterestByFormattedAddressAsync` is plain LINQ — a single `EF.Functions.ILike(FormattedAddress, '%term%')` that hard-codes `TagId`/`TagName` to `null`, so **search results never carry tags**. It is also the only POI read that cannot reach `Description` or tag text, which is the gap the unimplemented SOLR spec (`prompts/todo/implement search.md`) exists to close.
...
5. Three messages are published, all carrying that same `CorrelationId`: `append-formatted-address`, `resize-image`, `append-image-tags`. Each publish writes a `Published` audit row.
...
8. `AppendImageTags` consumer sends the original image to Claude and writes `Description` + tags (§7.5). **Not idempotent** — it has no "already described" guard, so a redelivery re-calls the Anthropic API.
...
1. POI creation publishes `append-image-tags` (§7.1 step 5). No user action is involved — there is no "Describe" button and no describe endpoint any more.
...
3. It fetches the **original** image (bucket `uploaded-images`, not the thumbnail) as base64 and calls `AnthropicImageDescriptionService.DescribeAsync`.
4. That service calls Claude with structured output (a JSON schema pinned to `{ description, tags[] }`, `MaxTokens = 512`), using model `AnthropicModel` (default `claude-haiku-4-5`). The response is a one-sentence description (max ~20 words) plus 3–6 single-word lowercase scene tags.
5. **Both are persisted**: `Description` is written with an explicit `EntityState.Unchanged` + `IsModified` toggle (the context is globally `NoTracking`), then tags go through `UpdatePointOfInterestTagsAsync` → `spUpdatePointOfInterestTags` if the list is non-empty.
6. `PoiDialog` renders `description`/`tags` straight off the polled POI payload.
   - If Anthropic is unconfigured (key unset or still the `<YOUR_ANTHROPIC_API_KEY>` placeholder), `DescribeAsync` throws `ApiException(503, …)` on every message — which, inside a consumer, means 3 retries then a `Failed` audit row per POI, not a clean skip.
...
1. `PointOfInterestService` mints one `CorrelationId` per POI, stores it on the row, and stamps it on all three messages (on re-upload via `UpdatePointOfInterestAsync`, the existing `CorrelationId` is reused, or minted if the row predates the column).
...
| `AnthropicApiKey` | `<YOUR_ANTHROPIC_API_KEY>` (placeholder) | **no — stale key, `api` never uses it** | nothing; `api` registers `IImageDescriptionService` only to satisfy `PointOfInterestService`'s constructor and never calls it. The live setting is on `messaging` (§10.2) |
...
- **`PointOfInterestMcpTools.ToDto` silently drops `Description` and `CorrelationId`** even though `PointOfInterestDto` carries both and the REST path populates them — an MCP client searching POIs sees strictly less than an HTTP client running the equivalent query. `src/api/mytravels.mcp/Tools/PointOfInterestMcpTools.cs` (`ToDto`).
...
| **Fanout exchange** | RabbitMQ exchange type that broadcasts to every bound queue, ignoring routing key — used for all six of this app's exchanges |
