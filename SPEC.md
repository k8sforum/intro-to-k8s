...
The two surfaces are **not feature-equivalent**: the REST API exposes CRUD operations (list, fetch, search, update, upload) plus the traceability read surface, none of which have an MCP counterpart. The MCP server is read-only search: `search_pointofinterest` (SOLR-backed search over saved POIs' address, tags and description, with optional tag and date-range filters) and `search_place` (place lookup) — an MCP client can only query existing data, not create or modify anything. The earlier `upload_photo` / `upload_photo_with_coordinates` tools were removed, which also retired the validation-drift and error-handling findings that applied to them (former F-16 and F-17).
...
| GET | `/api/pointofinterest/filter?filterString=` | query `filterString` (an **exact** tag name; resolves through SOLR since `api:v1.0.11`) | `List<PointOfInterestDto>` | 200 | none |
| GET | `/api/pointofinterest/search?term=&tag=&from=&to=&rows=&start=` | query `term` (free text over address, tags and description), optional `tag` (exact), `from`/`to` (capture-date range), `rows` (default 100), `start` | `List<PointOfInterestDto>` | 200 | none |
...
|---|---|---|---|
| `search_pointofinterest` | `term` (note: **not** `query`), plus optional `tag`, `from`, `to` | `List<PointOfInterestDto>` | `McpException` only when *all* of `term`, `tag`, `from` and `to` are absent — a tag-only or date-only search is valid. SOLR-backed since `mcp:v1.0.2`: free text is ranked across formatted address, tags and description, `tag` filters on an exact tag name, and `from`/`to` bound the capture date. Rows are grouped into one DTO per POI with its tags. |
...
8. `AppendImageTags` consumer sends the original image to Claude and writes `Description` + tags (§7.5), then publishes `index-solr` — so the document that carries the description and tags is only indexed after the Anthropic call succeeded. **Not idempotent** — it has no "already described" guard, so a redelivery re-calls the Anthropic API.
...
6. It then publishes `index-solr` with the inbound `CorrelationId`, because `description` and `tags` are two of the three fields SOLR ranks on and both were empty when the POI was first indexed at creation (§9).
7. `PoiDialog` renders `description`/`tags` straight off the polled POI payload.
...
- **Apache SOLR 9.10.1** — a **derived** store, not a system of record: the sole backend for POI search and the tag filter, rebuildable from PostgreSQL at any time. Single instance, one collection (`mytravels-pois`), no SolrCloud and no replicas; a PVC (Compose volume in stages 0–2, `local-path` PVC in stages 3–4) is the only durability, and `POST /api/pointofinterest/reindex` is the recovery path.
  ...
  - **Fields**: `id` (the `PointOfInterestKey`, uniqueKey), `poi_id`, `formatted_address`, `tags` (multiValued, tokenised), `tag_exact` (multiValued string, for the `/filter` path), `tag_ids` (multiValued int), `description`, `date_taken`, `date_created`, `latitude`, `longitude`, `container`, `original_file_name`, `generated_blob_name`, `image_resized`, `correlation_id`. `tags`/`tag_exact`/`tag_ids` are written in the same order and must stay in sync — both `ToDto` helpers discard any tag whose `TagId` is null, so the search side has to pair a name back up with its PostgreSQL id.
...
- **Read-your-writes**: there is none between PostgreSQL and SOLR. A POI is visible through `GET /api/pointofinterest` (PostgreSQL) as soon as the upload returns, but only appears in `/search` once the `index-solr` consumer has run, and its address/description/tags only once the corresponding enrichment subscriber has finished and republished. Updates commit with `commit=true` per batch, so there is no soft-commit delay on top of that.
...
- **SOLR search silently caps results at `rows` (default 100)** where the PostgreSQL `ILIKE` returned every match, and `GET /api/pointofinterest/filter` has no way to page at all — it calls `SearchAsync(new SolrSearchQuery { Tag = ... })` and so inherits the same default with no `rows` parameter on its route. A library with more than 100 POIs sharing a tag silently returns a truncated list with no indication it was truncated. `src/common/mytravels.contract/Dtos/SolrSearchQuery.cs`, `src/common/mytravels.domain/Features/PointOfInterest/PointOfInterestService.cs` (`GetAsync(string tagName, …)`).
- **The `/filter` endpoint's matching semantics changed** from a substring match over tag names *or* the formatted address (`spGetPointOfInterestByTagName` used `ILIKE '%term%'` on both) to an exact tag-name match via SOLR's `tag_exact` string field. Callers passing a partial tag, a differently-cased tag, or an address fragment now get an empty list rather than results. No caller in this repo does — the web app never calls `/filter` — but the route and response shape are unchanged, so the change is invisible to anything that does. `src/common/mytravels.common/Services/SolrSearchService.cs` (`fq` construction).
